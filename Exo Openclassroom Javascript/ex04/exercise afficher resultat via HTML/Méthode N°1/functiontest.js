...
function afficherResultat(resultat,NbElement) {//creation de la fonction pour afficher le resultat final et le nombre d'élément propose par le systeme
    ...
    let P1=document.createElement("div") //créer un variable et dedans on crée un nouvelle élément <div></div>
    P1.innerText=`${resultat}/${NbElement}`  //on introduit la valeur sortie de paramètre résultat et NbElement en provenance de la fonction
    let corps=document.querySelector("body")  //on récupère le DOM body de HTML puis on l'a nommé corps 
    corps.appendChild(P1)  //ajouter un élément fils P1 (${resultat}/${NbElement}) dans le variable corps pour l'afficher dans la page web
}


//ci-dessus c'est pour créer une nouvelle section <div> et on met le resultat et NbElement dedans, puis on met ce <div> comme un élément fils de <body>
