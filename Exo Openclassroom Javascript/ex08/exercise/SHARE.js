...
function validerNom(ContenuDeName) {
    ...
    return regex.test(ContenuDeName) //on teste le ContenuDeName avec la méthode regex.test, puis on retourne le résultat de cette fonction
}
...
function validerEmail(ContenuDeEmail) {
    ...
    return regex.test(ContenuDeEmail) //idem que la function ValiderNom
}
...
function SHARE(){
    ...
    FORM.addEventListener("submit",(event) =>{  
        ...
        let resultat1=validerNom(Nom.value)  //On crée un nouveau variable au sein de event "submit" pour la form id="send", resultat1 récupère le resultat retourné par la function ValiderNom avec le paramètre Nom.value (le Nom saisi dans le champ Votre Nom)
        let resultat2=validerEmail(Email.value) //On crée un nouveau variable au sein de event "submit" pour la form id="send", resultat2 récupère le resultat retourné par la function ValiderEmail avec le paramètre Email.value (l'Email saisi dans le champ "Avec qui vous voulez partager votre Score?")
        if (resultat1 === true && resultat2 === true){  //si resultat1 === true et resultat === true c à d, la valeur saisie correspond à la critère de RegExp, attention on n'ajoute pas "" pour true, sinon ça va pas marcher.
            console.log(Email.value)  //on imprime l'adresse mail saisi dans le champ "Avec qui vous voulez partager votre Score?" vers le console
        }else{
            console.log("message d'erreur") //sinon on imprime le mot "message d'erreur"
        }
    })
